...
import { PrincipalOpt, PrincipalReq, OptRespPromise, MakeOpt, Resp } from '@/utils/converter';

export const useAMM = () => {
  const [tokenInfo, setTokenInfo] = useState<Resp<AMM['get_token_info']> | null>(null);
  const [liquidityInfo, setLiquidityInfo] = useState<Resp<AMM['get_liquidity_info']> | null>(null);
  const [reserves, setReserves] = useState<Resp<AMM['get_reserves']> | null>(null);
  ...
  const handleGetQuote = useCallback(async (tokenInId: string, amountIn: bigint): OptRespPromise<AMM['get_swap_quote']> => {
  ...
  const handleSwap = useCallback(async (tokenInId: string, amountIn: bigint, minAmountOut: bigint): OptRespPromise<AMM['swap']> => {
    if (!checkDaoAmm()) return null;
    try {
  ...
  const handleAddLiquidity = useCallback(async (amount0: bigint, amount1: bigint): OptRespPromise<AMM['add_liquidity']> => {
    if (!checkDaoAmm()) return null;
    try {
